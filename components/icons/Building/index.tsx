import React from 'react'

export default function FacebookIcon({ width, height, color }: IconTypes) {
  return (
    <svg width={width ? width : "20"} height={height ? height : "20"} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M17 19V3C17 1.89543 16.1046 1 15 1H5C3.89543 1 3 1.89543 3 3V19M17 19L19 19M17 19H12M3 19L1 19M3 19H8M7 4.99998H8M7 8.99998H8M12 4.99998H13M12 8.99998H13M8 19V14C8 13.4477 8.44772 13 9 13H11C11.5523 13 12 13.4477 12 14V19M8 19H12" stroke={color ? color : "#CCCCD1"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
    


  )
}
